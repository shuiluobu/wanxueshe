...
(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    ...
    showCalendar: function () { // 输入数据并显示
      ...
      var today = new Date();
      // console.log("today:  "+returnDateStr(today)+"星期"+today.getDay()+"month:  "+month);
      //左上角日期    yyyy年MM月
      this.$calendarTitle_text.text(year + '年' + dateStr.substr(4, 2)+'月');
      //显示的日期 dd
      this.$calendarDate_item.each(function (i) {
        // allDay: 得到当前列表显示的所有天数
        // var allDay = new Date(year, month - 1, i + 1 - today.getDay());
        var allDay = new Date(year, month - 1, i + today.getDate() - today.getDay());
        // console.log( i +":   "+allDay.getDate() +"   -----"+(i + 1 - today.getDay()));
